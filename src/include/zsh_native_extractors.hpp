...
//==============================================================================
// Zsh-Specific Native Context Extractors
//==============================================================================

// Forward declaration for ZshAdapter
class ZshAdapter;

// Base template for Zsh extractors - default returns empty context
template <NativeExtractionStrategy Strategy>
struct ZshNativeExtractor {
	static NativeContext Extract(TSNode node, const string &content) {
		return NativeContext(); // Default: no extraction
	}
};

// Specialization for FUNCTION_WITH_PARAMS (Zsh functions)
template <>
struct ZshNativeExtractor<NativeExtractionStrategy::FUNCTION_WITH_PARAMS> {
	static NativeContext Extract(TSNode node, const string &content) {
		NativeContext context;

		try {
			// Infer return type from function body patterns and exit codes
			context.signature_type = InferZshReturnType(node, content);

			// Extract function parameters (Zsh functions use positional parameters)
			context.parameters = ExtractZshParameters(node, content);

			// Extract function attributes and visibility
			auto modifiers = ExtractZshModifiers(node, content);
...
private:
	static vector<ParameterInfo> ExtractZshParameters(TSNode node, const string &content) {
		...
		// In Zsh, function parameters are accessed as $1, $2, $3, etc.
		// We need to analyze the function body to determine what parameters are used
		string function_name = ExtractZshFunctionName(node, content);
	...
	static TSNode FindFunctionBody(TSNode function_node) {
		...
		for (uint32_t i = 0; i < child_count; i++) {
			...
			if (strcmp(child_type, "compound_statement") == 0 || strcmp(child_type, "do_group") == 0 ||
	...
	static vector<ParameterInfo> AnalyzeParameterUsage(TSNode body, const string &content) {
		...
		for (const auto &param : found_params) {
			ParameterInfo info;
			info.name = param;
			info.type = "";           // Zsh is untyped
			info.is_optional = false; // In Zsh, all positional params are technically optional
	...
	static vector<string> ExtractZshModifiers(TSNode node, const string &content) {
	...
	static string CheckFunctionSyntax(TSNode node, const string &content) {
		// Zsh functions can be declared as:
		// 1. function name() { ... }
		// 2. name() { ... }
	...
	static string ExtractZshFunctionName(TSNode node, const string &content) {
		// In Zsh function_definition, the name is typically found as an identifier
		uint32_t child_count = ts_node_child_count(node);
	...
	static string InferZshReturnType(TSNode node, const string &content) {
		// Infer return type from Zsh function patterns
		uint32_t child_count = ts_node_child_count(node);
		bool has_echo = false;
		...
		for (uint32_t i = 0; i < child_count; i++) {
			...
			if (start < content.length() && end <= content.length()) {
				...
				// Check for common Zsh output patterns
				if (child_text.find("echo") != string::npos || child_text.find("printf") != string::npos) {
					has_echo = true;
				} else if (child_text.find("return") != string::npos) {
					has_return = true;
				} else if (child_text.find("exit") != string::npos) {
					has_exit = true;
				} else if (child_text.find("$(") != string::npos || child_text.find("`") != string::npos) {
					has_command_substitution = true;
				} else if (child_text.find("=") != string::npos && child_text.find("local") == string::npos) {
					has_assignment = true;
				}
			}
		}
		...
		if (has_echo)
			return "string"; // Functions that echo output
		if (has_return)
			return "exit_code"; // Functions that return exit codes
		if (has_exit)
			return "exit_code"; // Functions that exit with codes
		if (has_command_substitution)
			return "command_output"; // Functions with command substitution
		if (has_assignment)
			return "side_effect"; // Functions with side effects

		return "exit_code"; // Default Zsh function return type
	}
};

// Specialization for VARIABLE_WITH_TYPE (Zsh variable declarations)
template <>
struct ZshNativeExtractor<NativeExtractionStrategy::VARIABLE_WITH_TYPE> {
	static NativeContext Extract(TSNode node, const string &content) {
		NativeContext context;

		// Zsh variables don't have explicit types, but we can infer declaration style
		context.signature_type = ExtractZshVariableType(node, content);

		// Extract variable modifiers and scope
		auto modifiers = ExtractZshVariableModifiers(node, content);
		context.modifiers = modifiers;
...
private:
	static string ExtractZshVariableType(TSNode node, const string &content) {
	...
	static vector<string> ExtractZshVariableModifiers(TSNode node, const string &content) {
		vector<string> modifiers;
	...
	static vector<string> ExtractDeclarationFlags(TSNode node, const string &content) {
		...
		for (uint32_t i = 0; i < child_count; i++) {
			...
			if (start < content.length() && end <= content.length() && end > start) {
				...
				if (text == "-a")
					flags.push_back("indexed_array");
				else if (text == "-A")
					flags.push_back("associative_array");
				else if (text == "-r")
					flags.push_back("readonly");
				else if (text == "-i")
					flags.push_back("integer");
				else if (text == "-x")
					flags.push_back("export");
				else if (text == "-u")
					flags.push_back("uppercase");
				else if (text == "-l")
					flags.push_back("lowercase");
			}
		}
